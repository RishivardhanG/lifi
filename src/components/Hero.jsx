...
// --- 1. THE PHOTONIC GRID (Li-Fi Visualization) ---
// Represents the high-speed modulation of light
const LifiGrid = () => {
  const mesh = useRef();
  const lightRef = useRef();
  const count = 60; // 60x60 grid
  const sep = 0.8; 
  ...
  useFrame((state) => {
    ...
    for (let xi = 0; xi < count; xi++) {
      for (let zi = 0; zi < count; zi++) {
        ...
        // Simulating High-Frequency Light Modulation
        // Faster frequency (t * 2), smaller amplitude to look like digital noise/data
        const y = Math.sin(x * 0.5 + t * 2) * Math.cos(z * 0.5 + t) * 0.8;
        
        // Distance from center for fade-out effect
      ...
      <div className="absolute inset-0 z-0 opacity-60">
        <Canvas dpr={[1, 2]}>
            ...
            <EffectComposer disableNormalPass>
               {/* Clean, sharp bloom for "Laser/Light" feel */}
               <Bloom luminanceThreshold={0.5} mipmapBlur intensity={1.8} radius={0.4} />
               {/* Slight Chromatic Aberration for the "Spectrum" feel */}
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
                ...
                <div className="flex flex-wrap gap-4">
                    <button className="px-8 py-3.5 bg-cyan-500 hover:bg-cyan-400 text-black font-semibold rounded text-sm tracking-wide transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.5)]">
                        DEPLOY PILOT
                    </button>
                    <button className="px-8 py-3.5 border border-white/20 hover:border-white/50 text-white font-medium rounded text-sm tracking-wide transition-all backdrop-blur-sm bg-white/5">
                        VIEW TECHNICAL SPECS
                    </button>
                </div>
            </motion.div>

            {/* RIGHT: Visual / Decorative (Optional space for 3D to breathe) */}
            <div className="hidden lg:block"></div>
         </div>
      </div>

      {/* --- BOTTOM HUD / STATS --- */}
      <div className="absolute bottom-0 left-0 w-full border-t border-white/5 bg-[#02040a]/80 backdrop-blur-md z-20">
